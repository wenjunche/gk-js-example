(function(window){
	...
	var SERVER_STATUS = "ws://localhost/";
	var REFERENCE_HANDLER = "ws://localhost/";
					...
					if (item instanceof Blob) {
						if (item.size > 0) {
							...
							recorder.getBuffer(function(blob) {
								myarray = new Float32Array(blob[0]);
								mymax = Math.max.apply(Math, myarray);
								//mymin = Math.min.apply(Math, myarray);
								//mymax = Math.max.apply(Math, [mymax, Math.abs(mymin)]);
								config.onEvent(MSG_SEND, '{"amplitude": ' + (mymax * 32767) + '}');
							});
						} else {
